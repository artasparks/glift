...
module('glift.displays.cropboxTest');
  ...
  test('For half boards', function() {
    ...
    deepEqual(lbox.widthIntersections(), 10.5 + overf, 'width ext for LEFT');
    deepEqual(lbox.heightIntersections(), 18 + overf, 'height for LEFT');

    deepEqual(rbox.bbox().topLeft().x(), 8, 'left coord for RIGHT');
    deepEqual(rbox.widthIntersections(), 10.5 + overf, 'width for RIGHT');
    deepEqual(rbox.heightIntersections(), 18 + overf, 'height for RIGHT');

    deepEqual(tbox.bbox().botRight().y(), 10, 'bottom coord for TOP');
    deepEqual(tbox.heightIntersections(), 10.5 + overf, 'height for TOP');
    deepEqual(tbox.widthIntersections(), 18 + overf, 'width for TOP');

    deepEqual(bbox.bbox().topLeft().y(), 8, 'top coord for BOTTOM');
    deepEqual(bbox.heightIntersections(), 10.5 + overf, 'height for BOTTOM');
    deepEqual(bbox.widthIntersections(), 18 + overf, 'width for BOTTOM');
  });
  ...
  test('Cropbox tests for quarter boards', function() {
    ...
    deepEqual(tlbox.widthIntersections(), 11.5 + overf, 'width for TOP LEFT');
    deepEqual(tlbox.heightIntersections(), 10.5 + overf, 'height for TOP LEFT');

    deepEqual(trbox.bbox().topLeft().x(), 7, 'left coord for TOP RIGHT');
    deepEqual(trbox.bbox().botRight().y(), 10, 'bottom coord for TOP RIGHT');
    deepEqual(trbox.widthIntersections(), 11.5 + overf, 'width for TOP RIGHT');
    deepEqual(trbox.heightIntersections(), 10.5 + overf, 'height for TOP RIGHT');

    deepEqual(blbox.bbox().botRight().x(), 11, 'right coord for BOTTOM LEFT');
    deepEqual(blbox.bbox().topLeft().y(), 8, 'top coord for BOTTOM LEFT');
    deepEqual(blbox.widthIntersections(), 11.5 + overf, 'width for BOTTOM LEFT');
    deepEqual(blbox.heightIntersections(), 10.5 + overf, 'height for BOTTOM LEFT');

    deepEqual(brbox.bbox().topLeft().x(), 7, 'left coord for BOTTOM RIGHT');
    deepEqual(brbox.bbox().topLeft().y(), 8, 'top coord for BOTTOM RIGHT');
    deepEqual(brbox.widthIntersections(), 11.5 + overf, 'width for BOTTOM RIGHT');
    deepEqual(brbox.heightIntersections(), 10.5 + overf, 'height for BOTTOM RIGHT');
  });
})();
